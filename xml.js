...
function getTagsValue(domObj,tagName){
	
	var childrenObj=domObj.childNodes;
	var childLength=childrenObj.length;
	
	for(var i=0;i<childLength;i++){
		var nodeObj=childrenObj[i];
		var isChildren=0;
		
		if(nodeObj.childNodes.length){
		  	isChildren=1;
		}
		
	    if(nodeObj.nodeName==tagName){           
		   var childObj=nodeObj.childNodes[0];		   
		   xmlArray.push(childObj.data);
		   continue;		   
		}
		
		if(isChildren){
		  analysis(nodeObj,tagName);
		}
